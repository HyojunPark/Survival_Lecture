...
map_tract <-
  ...
  tm_layout(title = "Obesity Rates by Census tracts"
            , title.size = 1.0
...
map_county <-
  ...
  tm_layout(title = "Obesity Rates by County"
            , title.size = 1.0
            , title.position = c("left", "top")
            , inner.margins = c(0.10, 0.00, 0.10, 0.00) #bottom, left, top, and right margin
            #, legend.position = c("center", "bottom")
...
Desc01 <- UT %>%
  reframe(
    ...
    , "Description" = measure
    , "Estimates (Tract)" = obesity_tract
    , "Estimates (County)" = obesity_county
  ) %>%
  DT::datatable(
    .
    , caption = "Table"
...
tmap_arrange(map_tract, map_county, ncol=2)
